...
-- Present here for test/bootstrap parity only. Added 0007; keep in sync with
-- migrations/0007_email_adapter.sql.
...
-- keep in sync with migrations/0012_risk_alerts.sql
-- Sprint Risk Board Phase 2: per-ticket alert hysteresis state + per-user opt-out.
-- Touched ONLY by worker/src/risk/store.ts via env.DB. Added 0012.
...
CREATE TABLE IF NOT EXISTS risk_alert_prefs (
  account_id  TEXT PRIMARY KEY,
  muted       INTEGER NOT NULL DEFAULT 0,   -- 1 = opted out of struggling-ticket nudges
  updated_at  TEXT NOT NULL
);
